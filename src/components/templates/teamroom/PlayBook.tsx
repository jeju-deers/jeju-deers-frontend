...
import { BoardTitleWrap, BoardWrap, PlayBookLayout, WriteButtonWrap } from "./PlayBookStyles";
...
const PlayBook = () => {
  const { data: playBookData = [], isLoading, error } = useGetBoards("play");
  const [filteredBoardlist, setFilteredBoardlist] = useState(playBookData);
  ...
  useEffect(() => {
    setFilteredBoardlist(playBookData);
  }, [playBookData]);

  const showSearch = () => {
    const result = handleSearch(playBookData, searchQuery, searchFilterOption);
