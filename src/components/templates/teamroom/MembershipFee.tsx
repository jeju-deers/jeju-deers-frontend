...
const MembershipFee = () => {
  const { data: membershipFeeData = [], isLoading, error } = useGetBoards("membership");
  const [filteredBoardlist, setFilteredBoardlist] = useState(membershipFeeData);
  ...
  useEffect(() => {
    setFilteredBoardlist(membershipFeeData);
  }, [membershipFeeData]);

  const showSearch = () => {
    const result = handleSearch(membershipFeeData, searchQuery, searchFilterOption);
